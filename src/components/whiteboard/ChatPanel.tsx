...
export default function ChatPanel({
  ...
  return (
    <div
      className="absolute right-4 bottom-4 z-20 w-72 bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col overflow-hidden animate-in slide-in-from-right-2"
      style={{ height: '380px' }}
    >
      <div className="flex items-center justify-between px-3 py-2 border-b border-gray-100 bg-gray-50 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          {messages.length > 0 && (
            <span className="text-[10px] px-1.5 py-0.5 bg-gray-200 text-gray-500 rounded-full">{messages.length}</span>
          )}
        </div>
          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
                {!isOwn && (
                  <span className="text-[11px] font-medium text-gray-500 mb-0.5">{msg.username}</span>
                )}
                <div className={`max-w-[85%] px-3 py-1.5 rounded-xl text-sm break-words ${
                  isOwn ? 'bg-gray-800 text-white rounded-br-sm' : 'bg-gray-100 text-gray-800 rounded-bl-sm'
                }`}>
                  {msg.content}
                </div>
                <span className="text-[10px] text-gray-400 mt-0.5">{formatTimestamp(msg.timestamp)}</span>
              </div>
            );
          })
        )}
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-t border-gray-100 bg-gray-50 flex-shrink-0">
        <Input
          ...
          maxLength={500}
        />
        <Button size="icon" className="h-8 w-8 rounded-lg flex-shrink-0" onClick={handleSend} disabled={!input.trim()}>
