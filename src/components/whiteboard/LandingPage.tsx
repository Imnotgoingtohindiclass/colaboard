    ...
    <div className="w-full min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Collab Board</h1>
          <p className="text-gray-500 mt-2 text-sm">
            Real-time collaborative whiteboard. Draw together, no sign-up required.
          </p>
        </div>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-800 truncate">{session.username}</p>
                <p className="text-xs text-gray-400">Your identity for this session</p>
              </div>
              <div className="w-4 h-4 rounded-full border border-gray-200 flex-shrink-0" style={{ backgroundColor: session.color }} />
            </div>
            ...
            {boardInput.trim().length > 0 && boardInput.trim().length < 4 && (
              <p className="text-xs text-amber-600">Board ID must be at least 4 characters</p>
            )}
          </CardContent>
        </Card>
        ...
        <div className="mt-8 grid grid-cols-3 gap-4">
          ...
          <FeatureCard icon={<Zap className="h-4 w-4" />} title="Low Latency" desc="CRDT-based sync via WebSocket" />
          <FeatureCard icon={<Shield className="h-4 w-4" />} title="No Login" desc="Just open and start drawing" />
        </div>
      </div>
    </div>
  );
...
function FeatureCard({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div className="text-center p-3">
      <div className="inline-flex items-center justify-center w-8 h-8 bg-gray-100 rounded-lg mb-2 text-gray-500">{icon}</div>
